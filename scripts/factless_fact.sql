...
--
CREATE VIEW product_launch_date_dim AS
SELECT DISTINCT
    date_sk AS product_launch_date_sk,
    date AS product_launch_date,
    month_name,
    month,
    quarter,
    year,
    ...
    b.expiry_date
FROM product_dim a
JOIN date_dim b ON a.effective_date = b.date;
--
SELECT table_name
FROM information_schema.tables
WHERE table_name LIKE 'product_%date_dim'
AND table_schema = 'public';
--
Select * From product_launch_date_dim
--
...
FROM product_dim a
JOIN date_dim b ON a.effective_date = b.date
GROUP BY a.product_sk, b.date_sk
HAVING COUNT(a.product_code) = 1;
--INSERT INTO product_count_fact (product_sk, product_launch_date_sk)
SELECT
  a.product_sk,
  MIN(b.date_sk) AS date_sk
FROM
  product_dim a
JOIN
...
SET expiry_date = '2007-03-20'::DATE - 1 -- Perhatikan Current Date
...
SELECT
  ...
  b.shipping_city,
  b.shipping_state,
  '2007-03-20'::DATE, -- Perhatikan Current Date
  '9999-12-31'::DATE
FROM
  customer_dim a
...
WHERE (
  ...
  AND EXISTS (
    ...
    WHERE b.customer_number = x.customer_number
      AND a.expiry_date = '2007-03-20'::DATE - 1) -- Perhatikan Current Date
  AND NOT EXISTS (
    SELECT *
    FROM customer_dim y
...
SELECT
  ...
  shipping_city,
  shipping_state,
  '2007-03-20', -- Perhatikan Current Date
  '9999-12-31'
FROM customer_stg
...
SET expiry_date = '2007-03-20'::DATE - 1
...
SELECT
  ...
  b.product_name,
  b.product_category,
  '2007-03-20'::DATE, -- Perhatikan Tanggal Current Date A
  '9999-12-31'
FROM
  product_dim a
JOIN
...
AND EXISTS (
  ...
  AND a.expiry_date = '2007-03-20'::DATE - 1 ) -- Perhatikan Tanggal Current Date
AND NOT EXISTS (
  SELECT *
  FROM product_dim y
...
WHERE
  a.order_status = 'S'
  AND b.entry_date = '2007-03-20' -- Perhatikan tanggal Current Date
  AND b.order_number = a.order_number;
...
WHERE
  ...
  AND b.entry_date = '2007-03-20' -- Perhatikan Tanggal Current Date
  AND b.order_number = a.order_number;
-- End Revies DW Population Script
SELECT 
    c.product_sk AS psk,
    c.product_code AS pc,
    b.product_launch_date_sk AS plsk,
    b.product_launch_date AS pld
FROM 
    product_count_fact a
JOIN 
    product_launch_date_dim b ON a.product_launch_date_sk = b.product_launch_date_sk
JOIN 
    product_dim c ON a.product_sk = c.product_sk
ORDER BY 
    c.product_code, b.product_launch_date;
